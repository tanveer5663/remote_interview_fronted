...
import useStreamClient from "../hooks/useStream.js";
...
function SessionPage() {
  ...
  const isHost = session?.host?.clerkId === userData?.clerkId;
  console.log("isHost", isHost);
  const isParticipant = session?.participant?.clerkId === userData?.clerkId;
  ...
  useEffect(() => {
    try {
      const fetchSessionData = async () => {
        ...
        // console.log("after fetching sesisonDAta", data);
  ...
  let problemData = sessionData?.session?.problem
    ? Object.values(PROBLEMS).find(
        (p) => p.title === sessionData?.session?.problem,
      )
    : null;
  ...
  const handleEndSession = async () => {
    if (
      ...
      try {
        const endSession = await sessionApi.endSession(id);
        console.log(endSession);
        // navigate("/dashboard");
      } catch (error) {
        console.log("error in handleEnd session");
      }
    }
  };
  return (
                  ...
                  <div className="p-6 bg-base-100 border-b border-base-300">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {sessionData?.session?.difficulty
                            .slice(0, 1)
                            .toUpperCase() +
                            sessionData?.session?.difficulty.slice(1) || "Easy"}
                        </span>
                        {!isHost &&
